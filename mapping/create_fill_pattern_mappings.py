# Dynamo Python (Revit, CPython3) -- fill_patterns mapping utility entry point.
#
# Creates representative FillPatternElement objects in the currently open Revit
...
# tooling. See docs/fill_pattern_mapping.md for the full design note and
# docs/fill_pattern_mapping_verification.md for the manual verification
# procedure this script's own post-creation checks automate.
#
# This script only WRITES FillPatternElement objects (via bounded, individually
# verified Transactions -- see mapping/fill_pattern_revit_apply.py). It never
...
#
# One entry point covers BOTH fill_patterns partitions (fill_patterns_drafting
# and fill_patterns_model, per D-015's domain-family split): the reconstruction/
# verification/creation logic is identical between them (only the expected
# fill_pattern.target value and join-key policy entry differ), and both read
# from the same export directory -- see docs/fill_pattern_mapping.md for why a
# second entry point would be pure duplication.
#
...
# loader shim needed to load that module before a repo root is known.
#
# Inputs:
#   IN[0] input_dir (str)
#        Directory containing bundle_pattern_inventory.csv, pattern_settings.csv,
#        and pattern_names.csv (the output of
#        tools/export_bundle_pattern_detail.py for one segment, covering either
#        or both of fill_patterns_drafting / fill_patterns_model).
#
#   IN[1] report_path (str)
#        Path to write the deterministic CSV report to (one row per requested
#        (domain, join_hash) across both partitions, plus one row per inventory
#        entry whose join_hash was blank -- see
#        mapping/fill_pattern_reconstruction.py's REPORT_FIELDS).
#
#   IN[2] repo_root (str, optional)
#        Absolute path to the Revit_Fingerprint checkout, needed only when
#        __file__ can't be used to find it and neither
...
from mapping.fill_pattern_revit_apply import build_name_index, resolve_mapping
...
def run(doc, input_dir, report_path):
    ...
    # created, so join_hashes processed later in this same run (including in the
    # second domain's loop) see name collisions against elements this run itself
    # just created. Shared across both partitions since they collide in the same
    # FillPatternElement namespace within one document.
    name_index = build_name_index(doc)

    outcomes = []
    for domain_name in DOMAIN_NAMES:
        requested, skipped = group_requested_join_hashes(export["inventory"], domain_name)
        settings_by_jh = group_settings_by_join_hash(export["settings"], domain_name)
        names_by_jh = group_names_by_join_hash(export["names"], domain_name)
        ...
        for join_hash in sorted(requested.keys()):
            ...
            reconstructed = reconstruct_pattern(domain_name, join_hash, settings_by_jh.get(join_hash, []))

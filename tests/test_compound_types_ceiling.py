# -*- coding: utf-8 -*-
"""Minimal coverage for domains/ceiling_types.py's sig_hash computation,
...
def _setup_module(monkeypatch):
    m = importlib.import_module("domains.ceiling_types")
    monkeypatch.setattr(m, "CeilingType", object)
    ...
    monkeypatch.setattr(m, "BuiltInCategory", type("_BIC", (), {"OST_Ceilings": "OST_Ceilings"}))
    return m
...
def _basic_ceiling(name="Ceiling A"):
    ...
    return _CeilingType(name=name, cs=cs)


def test_basic_ceiling_produces_ok_record(monkeypatch):
    m = _setup_module(monkeypatch)
    monkeypatch.setattr(m, "collect_types", lambda *a, **k: [_basic_ceiling("F1")])
    out = m.extract_ceiling_types(_Doc({101: "m1", 102: "m2", 103: "m3"}), _default_ctx())
    rec = out["records"][0]
...
def test_type_name_not_in_sig_hash(monkeypatch):
    m = _setup_module(monkeypatch)
    monkeypatch.setattr(m, "collect_types", lambda *a, **k: [_basic_ceiling("Name A"), _basic_ceiling("Name B")])
    out = m.extract_ceiling_types(_Doc({101: "m1", 102: "m2", 103: "m3"}), _default_ctx())
...
def test_sig_basis_keys_used_matches_fallback(monkeypatch):
    m = _setup_module(monkeypatch)
    monkeypatch.setattr(m, "collect_types", lambda *a, **k: [_basic_ceiling()])
    rec = m.extract_ceiling_types(_Doc({101: "m1", 102: "m2", 103: "m3"}), _default_ctx())["records"][0]
    assert set(rec["sig_basis"]["keys_used"]) == set(m._CEILING_TYPES_SIG_HASH_KEYS_FALLBACK)


def test_no_compound_structure_blocked(monkeypatch):
    m = _setup_module(monkeypatch)
    ceiling = _CeilingType(name="NoCS", cs=None)
    monkeypatch.setattr(m, "collect_types", lambda *a, **k: [ceiling])
    out = m.extract_ceiling_types(_Doc(), _default_ctx())
    rec = out["records"][0]
...
def test_sig_hash_reads_allowed_items_from_ctx_sig_hash_policies_when_present(monkeypatch):
    m = _setup_module(monkeypatch)
    monkeypatch.setattr(m, "collect_types", lambda *a, **k: [_basic_ceiling("Name A")])

    ctx = _default_ctx()
    ctx["sig_hash_policies"] = {
        "domains": {
            "ceiling_types": {
                "sig_hash_schema": "ceiling_types.sig_hash.v1",
                "hash_alg": "md5_utf8_join_pipe",
                "allowed_items": ["ct.layer_count"],
    ...
    out = m.extract_ceiling_types(_Doc({101: "m1", 102: "m2", 103: "m3"}), ctx)
    rec = out["records"][0]

    from core.hashing import make_hash
    from core.record_v2 import serialize_identity_items

    layer_count_item = [it for it in rec["identity_basis"]["items"] if it["k"] == "ct.layer_count"]
    expected = make_hash(serialize_identity_items(layer_count_item))
    assert rec["sig_hash"] == expected

    default_out = m.extract_ceiling_types(_Doc({101: "m1", 102: "m2", 103: "m3"}), _default_ctx())

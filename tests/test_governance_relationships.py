...
def test_non_project_roles_excluded():
    rows = [
        _row("f1", "ClientBeta", "2014", "Alpha", role="Project"),
        _row("f2", "InternalEnterprise", "2014", "n/a", role="Template"),
        _row("f3", "InternalEnterprise", "2014", "n/a", role="Container"),
    ]
    out, warnings = build_relationships_rows(rows)
    assert len(out) == 1
    assert out[0]["client_label"] == "ClientBeta"
    assert warnings == []
...
def test_lowercase_governance_role_still_counted_as_project():
    ...
    rows = [_row("f1", "ClientBeta", "2014", "Alpha", role="project")]
...
def test_client_label_casing_variants_fold_to_one_project():
    rows = [
        _row("f1", "ClientBeta", "2014", "Alpha", role="Project"),
        _row("f2", "clientbeta", "2014", "Alpha", role="Project"),
    ]
    out, _ = build_relationships_rows(rows)
    assert len(out) == 1
    assert out[0]["client_label"] == "ClientBeta"  # first-seen casing
...
def test_bc_prefix_variant_folds_to_same_bc_identity():
    ...
    rows = [
        _row("f1", "ClientBeta", "BC_2014", "Alpha", role="Project"),
        _row("f2", "ClientBeta", "2014", "Alpha", role="Project"),
    ]
    out, _ = build_relationships_rows(rows)
    assert len(out) == 1
...
def test_excel_collapsed_bc_folds_to_same_bc_identity():
    ...
    rows = [
        _row("f1", "ClientBeta", "0796", "Alpha", role="Project"),
        _row("f2", "ClientBeta", "796", "Alpha", role="Project"),
    ]
    out, _ = build_relationships_rows(rows)
    assert len(out) == 1
...
def test_same_project_label_different_client_stays_distinct():
    ...
    rows = [
        _row("f1", "ClientBeta", "2014", "MPMC"),
        _row("f2", "ClientBeta", "2014", "MPMC"),
        _row("f3", "Duke LifePoint", "2014", "MPMC"),
    ]
    out, _ = build_relationships_rows(rows)
    assert len(out) == 2
    clientbeta_mpmc = _find(out, client_label="ClientBeta", business_center_label="2014")
    duke_mpmc = _find(out, client_label="Duke LifePoint", business_center_label="2014")
    assert clientbeta_mpmc["project_name"] == "MPMC"
    assert clientbeta_mpmc["project_file_count"] == "2"
    assert duke_mpmc["project_name"] == "MPMC"
    assert duke_mpmc["project_file_count"] == "1"
    assert clientbeta_mpmc["project_id"] != duke_mpmc["project_id"]


def test_blank_project_label_falls_back_to_export_run_id_per_file():
    rows = [
        _row("f1", "ClientBeta", "2014", "__NOT_APPLICABLE__"),
        _row("f2", "ClientBeta", "2014", ""),
    ]
    out, _ = build_relationships_rows(rows)
...
def test_multi_discipline_project_collects_sorted_discipline_list():
    rows = [
        _row("f1", "ClientBeta", "2014", "Alpha", discipline="architectural"),
        _row("f2", "ClientBeta", "2014", "Alpha", discipline="structural"),
        _row("f3", "ClientBeta", "2014", "Alpha", discipline="electrical"),
    ]
    out, _ = build_relationships_rows(rows)
    assert len(out) == 1
...
def test_enterprise_bookkeeping_bc_token_blanked_not_carried_as_fake_bc():
    ...
    rows = [
        _row("f1", "ClientBeta", "0000", "Alpha", role="Project"),
        _row("f2", "ClientAlpha", "BC_0000", "Beta", role="Project"),
    ]
    out, warnings = build_relationships_rows(rows)
    assert len(out) == 2
    for r in out:
...
def test_enterprise_bookkeeping_project_excluded_from_bc_client_matrix():
    rows = [
        _row("f1", "ClientBeta", "0000", "Alpha", role="Project"),
        _row("f2", "ClientBeta", "2014", "Beta", role="Project"),
    ]
...
def test_inconsistent_unit_system_within_one_project_warns_not_raises():
    rows = [
        _row("f1", "ClientBeta", "2014", "Alpha", unit="imperial"),
        _row("f2", "ClientBeta", "2014", "Alpha", unit="metric"),
    ]
    out, warnings = build_relationships_rows(rows)
    assert len(out) == 1
    assert out[0]["project_file_count"] == "2"
...
def test_percentage_of_bc_and_client_single_bc_per_client():
    relationship_rows = [
        _rel_row("ClientBeta", "2014", 62),
        _rel_row("Renown", "2014", 41),
    ]
    matrix = build_bc_client_matrix_rows(relationship_rows)
    clientbeta = _find(matrix, client_label="ClientBeta")
    assert clientbeta["percentage_of_bc"] == f"{62/103:.6f}"
    assert clientbeta["percentage_of_client"] == "1.000000"
...
def test_percentage_of_client_sums_to_one_across_multiple_bcs():
    ...
    relationship_rows = [
        _rel_row("ClientBeta", "2014", 60),
        _rel_row("ClientBeta", "1779", 40),
        _rel_row("Renown", "2014", 41),
    ]
    matrix = build_bc_client_matrix_rows(relationship_rows)
    clientbeta_rows = [r for r in matrix if r["client_label"] == "ClientBeta"]
    assert len(clientbeta_rows) == 2
    total_pct = sum(float(r["percentage_of_client"]) for r in clientbeta_rows)
    assert abs(total_pct - 1.0) < 1e-9

    client_matrix = build_client_bc_matrix_rows(matrix)
    clientbeta = _find(client_matrix, client_label="ClientBeta")
    assert clientbeta["business_center_count"] == "2"
    # Ordered by percentage_of_client descending -- 2014 (60/100) before 1779 (40/100).
    assert clientbeta["business_centers"] == "2014|1779"
    assert clientbeta["project_file_count"] == "100"


def test_client_bc_matrix_never_recomputes_percentage_it_only_sums_counts():
    relationship_rows = [_rel_row("ClientBeta", "2014", 62), _rel_row("Renown", "2014", 41)]
    matrix = build_bc_client_matrix_rows(relationship_rows)
    client_matrix = build_client_bc_matrix_rows(matrix)

...
tools/                  Analysis & comparison utilities (no Revit dependency; stdlib CSV/JSON only)
  ...
  compare_reference.py   Standalone reference-vs-target comparison workflow. Orchestration/output layer
                            only (no comparison mathematics of its own) -- resolves --reference/--target
                            filename selectors against an already-materialized segment's own
                            results/records/file_metadata.csv (tools/run_segment_orchestrator.py's output),
                            then calls bundle_analysis/step_compare.py::run_compare_for_domain directly
                            in-process against that segment's own results/analysis/pattern_presence_file.csv
                            and results/bundle_analysis/{all,used}/<domain>/membership_matrix.csv. Reads no
                            fingerprint export JSON and never invokes run_extract_all.py; a segment that is
                            missing, incomplete, or internally inconsistent blocks explicitly rather than
...
- `docs/reference_comparison_tool.md` — standalone `tools/compare_reference.py` reference-vs-target comparison workflow: CLI reference, output schema, and ok/degraded/blocked interpretation
...
- **`comparison_registry.csv` bundle-run failures — root cause not confirmed, workaround is relocating off OneDrive.** `run_type=bundle` segment runs (`run_registry.csv`) can fail near-100% at the `clear_stale_name_all` step with `[WinError 5] Access is denied` on a path under `...\Fingerprint_Data\segments\<segment>\results\bundle_analysis\name_all\<domain>` — confirmed 2026-08-19: 73 of 74 bundle runs failed this way in one run, 74 of 74 in a follow-up run with OneDrive sync paused, while `run_type=reference` runs (28/28 both times) were unaffected. **OneDrive sync was the initial hypothesis and is not confirmed** — `retry_fs_op()` (`tools/bundle_analysis/common.py`) already retries this exact operation 5x with backoff specifically anticipating a transient OneDrive lock, and the failure persisted at the same rate past that retry window even with sync paused, which argues against a transient sync lock as the sole or primary cause. A persistent handle from something else (Power BI Desktop/Tabular Editor with a live connection into `name_all/`, given that folder explicitly exists to feed Power BI; a lingering process from a prior failed run; AV real-time scanning) is equally or more likely and was not ruled out — `handle.exe -a <path>` against a live failure would identify the actual holding process definitively, but this hasn't been run yet. **Current workaround:** relocate `Fingerprint_Data` outside any OneDrive-synced folder and point the runbook at it with `-ExportsRoot`; this sidesteps the failure class regardless of which process was actually responsible. `comparison_registry.csv` is stamped almost entirely by successful `bundle`/`reference` runs, so this failure mode is *why* `governance_package_health.json`'s `comparison_completeness` shows heavy "missing" counts even though `cross_segment_summary.csv` has full evidence — the comparisons ran, the registry-stamping step for them didn't.

# -*- coding: utf-8 -*-
"""
Object Styles (Analytical) domain extractor.

Domain family: object_styles
Contains: CategoryType.AnalyticalModel categories and subcategories

Per-record identity: sig_hash (UID-free) derived from identity_items.
Ordering: order-insensitive (sorted before hashing)

Cut weight is NOT applicable to Analytical categories — do not emit it.
"""
...
DOMAIN_NAME = "object_styles_analytical"

# Semantic keys for sig_hash preimage — no weight.cut for Analytical
...
def _is_analytical_category_type(ct):
    """
    Check whether a CategoryType value is the Analytical tab.

    Revit may expose this as CategoryType.AnalyticalModel (newer API)
    or as a numeric value. We use attribute-name matching to be version-agnostic.
    """
    try:
        ct_str = safe_str(ct).strip().lower()
        return "analytical" in ct_str
    except Exception:
        return False
...
def extract(doc, ctx=None):
    """
    Extract Object Styles (Analytical) fingerprint from document.

    Includes CategoryType.AnalyticalModel categories and subcategories only.
    Does NOT emit obj_style.weight.cut (not applicable to Analytical).
    """
    info = {
        "count": 0,
        "raw_count": 0,
    ...
    for cat in list(cats or []):
        ...
        try:
            ct = cat.CategoryType
            if not _is_analytical_category_type(ct):

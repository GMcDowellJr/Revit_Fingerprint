# -*- coding: utf-8 -*-
"""
View Templates domain extractor - Floor Plans, Structural Plans, and Area Plans.

Filters to only process view templates whose ViewType is one of:
  FloorPlan, StructuralPlan, AreaPlan
...
_HANDLED_VIEW_TYPES = frozenset({"FloorPlan", "StructuralPlan", "AreaPlan"})


def extract(doc, ctx=None):
    """
    Extract view templates fingerprint - Floor Plans, Structural Plans, Area Plans only.
    ...
    for v in col:
        ...
        # Check ViewType filter - only process FloorPlan, StructuralPlan, AreaPlan
        try:
        ...
        # -----------------------------------------
        # NON-SCHEDULE templates (FloorPlan / StructuralPlan / AreaPlan are never schedules)
        # -----------------------------------------
        sig = []

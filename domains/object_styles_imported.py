# -*- coding: utf-8 -*-
"""
Object Styles (Imported) domain extractor.

Domain family: object_styles
Contains: CategoryType.Import categories (CAD imports, DWG/DXF/SAT)

Per-record identity: sig_hash (UID-free) derived from identity_items.
Ordering: order-insensitive (sorted before hashing)

Cut weight is NOT applicable to Imported categories — do not emit it.
Pattern reference is included as optional (q=missing if not set).
"""
...
DOMAIN_NAME = "object_styles_imported"

# Semantic keys for sig_hash preimage — no weight.cut for Imported
...
def extract(doc, ctx=None):
    """
    Extract Object Styles (Imported) fingerprint from document.

    Includes CategoryType.Import categories and subcategories only.
    Does NOT emit obj_style.weight.cut (not applicable to Imported).
    """
    info = {
        "count": 0,
        "raw_count": 0,

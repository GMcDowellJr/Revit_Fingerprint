# -*- coding: utf-8 -*-
"""Model partition for view category overrides."""
...
DOMAIN_NAME = "view_category_overrides_model"
...
def extract(doc, ctx=None):
    ...
    baseline_sig_map = (ctx or {}).get("object_style_row_key_to_sig_hash") or {}
    ...
    try:
        if _tc is not None:
            try:
                _tc.start_timer("api:vco_model.enumerate_categories")
            except Exception:
                pass
        ...
        if _tc is not None:
            try:
                _tc.end_timer("api:vco_model.enumerate_categories")
            except Exception:
                pass
    ...
    for template in templates:
        ...
        for (cat, is_sub, parent) in all_cats:
            ...
            coordination = [
                make_identity_item("vco.vg_tab", "Model", ITEM_Q_OK),

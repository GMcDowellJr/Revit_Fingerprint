...
def emit_element_dominance(analysis_dir: Path, domain: Optional[str] = None) -> Path:
    ...
    authority_csv = analysis_dir / "authority_patterns.csv"
    ...
    if not domain:
        threshold_run_ids = {row.get("analysis_run_id", "") for row in threshold_rows}
        missing_run_ids = sorted(run_ids_seen - threshold_run_ids)
...
def main() -> None:
    ...
    parser.add_argument("--analysis-dir", required=True, help="Path to results/analysis directory.")

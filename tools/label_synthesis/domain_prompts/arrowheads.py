...
NAMING RULES
============
1. Always start with the style type.
2. For Arrow class: fill state (Filled/Open) is the most important qualifier.
3. For SizeOnly class: style + size is the complete identity.
   If only one size exists in corpus, size can be omitted.
4. Include size when multiple sizes of the same style appear in the corpus.
5. Keep names under 40 characters for BI slicer legibility.
6. If observed names are messy variants (Arrow Filled vs ARROW-SM vs Standard Arrow),
   synthesize the cleanest canonical form they were trying to use.
"""

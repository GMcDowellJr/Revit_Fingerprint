...
def compute_named_cluster_flags(summary_df: pd.DataFrame) -> pd.Series:
    ...
    share_col = 'raw_share' if 'raw_share' in summary_df.columns else 'percentage'
    sorted_df = summary_df.sort_values(share_col, ascending=False).reset_index(drop=True)
    shares = sorted_df[share_col].astype(float).values
    ...
    *,
    ...
    for cluster in clusters:
        ...
        summary.append({
            ...
            'standard_name': std_name,
            'size': cluster.size,
            'raw_share': (cluster.size / len(file_profiles)),
    ...
    summary_df = summary_df[[
        'cluster_id', 'standard_name', 'size', 'raw_share', 'percentage',

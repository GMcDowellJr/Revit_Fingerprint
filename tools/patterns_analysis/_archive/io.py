...
def load_records_file_paths(phase0_dir: str) -> Dict[str, str]:
    """Return best-effort file_id/export_run_id -> path for metadata heuristics.

    Uses results/records/file_metadata.csv if present.
    """
...
def load_records_sig_profiles(phase0_dir: str, domain: str) -> Tuple[Dict[str, Dict[str, Any]], Dict[str, str]]:
    ...
    """
    phase0_dir = os.path.abspath(phase0_dir)
    rec_csv = os.path.join(phase0_dir, "records.csv")
    if not os.path.isfile(rec_csv):
        raise FileNotFoundError(f"records.csv not found: {rec_csv}")

    domain = str(domain)
    file_paths = load_records_file_paths(phase0_dir)
...
def load_records_records_with_identity(
    phase0_dir: str,
    domain: str,
    *,
    allowed_file_ids: Optional[Set[str]] = None,
) -> List[Dict[str, Any]]:
    """Load v2.1 Phase0 records and attach identity_basis.items from identity_items.csv.
    ...
    phase0_dir = os.path.abspath(phase0_dir)
    rec_csv = os.path.join(phase0_dir, "records.csv")
    items_csv = os.path.join(phase0_dir, "identity_items.csv")

    if not os.path.isfile(rec_csv):
        raise FileNotFoundError(f"records.csv not found: {rec_csv}")
    if not os.path.isfile(items_csv):
        raise FileNotFoundError(f"identity_items.csv not found: {items_csv}")

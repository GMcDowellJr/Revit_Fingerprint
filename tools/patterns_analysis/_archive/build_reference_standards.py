# tools/patterns_analysis/build_reference_standards.py
...
def build_reference_standards_from_clusters(
    ...
    *,
    ...
    for std_name in standards:
        ...
        if phase0_dir:
            # CSV mode: read sig_hashes from results/records/records.csv
            rec_csv = os.path.join(os.path.abspath(phase0_dir), "records.csv")
            if not os.path.isfile(rec_csv):
                sys.stderr.write(f"[WARN] records.csv not found: {rec_csv}\n")
                continue

...
# FloorFunction is not importable in all Revit API versions/surfaces -- matches
# the same defensive guard domains/compound_types.py already uses. If this
# fires, ft.function will show as unreadable in both the probe and the real
# pipeline; that's an environment fact, not a bug to fix here.
try:
...
# -------------------------
# Discovery -- ALL FloorTypes, no sampling
# -------------------------

all_types = _safe(
    lambda: (FilteredElementCollector(doc).OfClass(FloorType).ToElements()),
    default=[]
)
try:
...
def _resolve_similar_type(sid_int):
    ...
    lookup, not a FloorType-only id->name dict -- GetSimilarTypes() is not
    formally documented as same-category-only (see Step 0 findings), so
    this must not assume the returned id is necessarily another FloorType."""
    if sid_int is None:
        return (None, False)
...
_reflection_records_type = _run_reflection_sweep(_reflect_type_samples, "FloorType", "floor_types")
_reflection_records_cs = _run_reflection_sweep(_reflect_cs_samples, "CompoundStructure", "floor_types")
_reflection_records_layer = _run_reflection_sweep(_reflect_layer_samples, "CompoundStructureLayer", "floor_types")
...
if write_json:
    try:
        ...
        with open(target_path, "w") as f:
            json.dump(_probe_wrap("floor_types", OUT_payload), f, indent=2, sort_keys=True)

...
def _iter_export_files(exports_dir: Path) -> List[Tuple[str, Path, Optional[Path]]]:
    ...
    sys.stderr.write(
        "[INFO extractor] export surfaces: "
...
def _load_governance_role_rules() -> List[Dict[str, str]]:
    try:
        ...
        sys.stderr.write(
            f"[WARN extractor] governance_role inference disabled: could not load "
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("rules"), list):
        sys.stderr.write(
            f"[WARN extractor] governance_role inference disabled: malformed rules file "
    ...
    for idx, rule in enumerate(payload.get("rules", [])):
        ...
        if not role or not path_contains:
            sys.stderr.write(f"[WARN extractor] skipping invalid governance rule at index {idx}\n")
            continue
...
def _load_identity_items_by_record(
    ...
    if domain:
        scoped = phase0_dir / "identity_items_by_domain" / f"{domain}.csv"
        if scoped.is_file():
            csv_path = scoped

    if csv_path is None:
        fallback = phase0_dir / "identity_items.csv"
...
def emit_records(exports_dir: Path, out_dir: Path, file_id_mode: str = "basename") -> Tuple[int, int]:
    ...
    Returns (file_count, record_count). High-volume CSVs (records, labels, reasons,
    parameter_rows) are written row-by-row so memory is bounded by a single export
    file's content, not the entire corpus. file_metadata.csv is written after the
    loop since it is small (one row per file) and needs annotation preservation.
    """
    ...
    # Set up identity-item shard dir (already streams per-domain).
    ...
    meta_rows: List[Dict[str, str]] = []  # one row per file — stays in memory
    record_count = 0
    ...
    with (
        ...
        for _w in (_rec_w, _lbl_w, _rsn_w, _par_w):
            _w.writeheader()
    ...
    # rebuild (see DECISIONS.md D-037).
...
def emit_analysis(
    ...
    *,
    ...
    _write_csv(out_dir / "export_membership.csv", [
    ...
    for _rank, _dt in enumerate(_domain_timings[:5], 1):
        sys.stderr.write(
            f"[patterns_timing] domain={_dt['domain']} elapsed={_dt['total']:.2f}s "
    ...
    _write_csv(out_dir / "authority_patterns.csv", [
    ...
    _write_csv(out_dir / "pattern_diagnostics.csv", [

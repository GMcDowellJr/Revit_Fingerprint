...
def main() -> None:
    ...
    manifest = read_csv(analysis_dir / "manifest.csv")
    if manifest:
        row = manifest[0]
        for k in ("join_key_policy_version", "pattern_promotion_policy_version", "authority_metric_version"):
            if not SEMVER_RE.match(row.get(k, "")):
                errors.append(f"manifest.csv invalid or missing semver field: {k}")
        if row.get("is_incremental_update", "") not in {"0", "1"}:
            errors.append("manifest.csv is_incremental_update must be 0/1")

...
**Verification uses the same effective gates as the search that produced the candidate.** `discover_greedy()` strips `gates["required_fields"]` from its own scoring calls once `selected` is required-inclusive (see the required-fields section above) -- the full-population re-score does the same (`verify_cfg`, built the identical way), rather than reusing the original `cfg` (which still has `gates["required_fields"]` set for validate/harsh modes). Reusing the original `cfg` here would make `score_candidate()`'s full-population pass silently fall back to scoring the required baseline alone, ignoring whatever challenger field greedy actually picked -- comparing sample metrics computed against the real `selected` against full metrics computed against a different, narrower key, which can manufacture a false divergence (or mask a real one) whenever `selected` differs from the required fields.

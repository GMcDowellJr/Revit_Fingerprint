...
| `apply` | T2 | Apply policy and overwrite flatten `phase0_records.csv` join fields. | ❌ No | N/A | Explicit opt-in for operational commit path. Defaults to `{results-root}/policies/domain_join_key_policies.v21.json` when `--join-policy` is not provided, where `{results-root}` is `{out-root}` itself if `--out-root-is-results-root` is passed, else `{out-root}/results/` (the default -- the canonical commands below all use this default). |

...
- `element_characterization_thresholds.csv` is a post-emit artifact generated by `tools/emit_element_dominance.py` after `element_dominance.csv` is written.
...
- `element_characterization_thresholds.csv` has a single row at corpus-level grain combining `object_styles_model`, `object_styles_annotation`, and `view_category_overrides`.

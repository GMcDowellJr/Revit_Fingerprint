...
- `multi_project` — the pattern appears in 3+ projects inside a client/domain pool.
...
- `emerging` — the pattern appears in 2+ files across exactly 2 projects inside a client/domain pool.
...
Thresholds are centralized in `tools/compare_cross_segment.py` as `REUSE_BUCKET_THRESHOLDS`. The current defaults are `corpus_wide_min_pct_clients=0.80`, `client_wide_min_pct_files=0.80`, `multi_project_min_projects=3`, and `emerging_min_files=2`.
...
One row per `(segment_id_a, segment_id_b, comparison_type, domain)` work item that actually produced a persisted output row this run — the same granularity as `work_items`, not the coarser `(segment_id_a, segment_id_b, comparison_type)` pair. This matters for scoped invocations: a `--domain line_patterns` run only recomputes `line_patterns` for each discovered pair, so stamping at pair granularity would silently mark every *other* domain on that pair "current" without having recomputed it — hiding real staleness from a later `--dry-run`. Keying on domain means a `--domain`-scoped run only stamps the domains it actually touched.
...
The file is a **full snapshot of this invocation only** — unlike `run_registry.csv`, it is never merged with a prior `comparison_registry.csv`. Every other output this tool writes (`cross_segment_summary.csv`, `cross_segment_file_pairs.csv`, ...) is a full `atomic_write_csv` replace from only this run's rows, never a merge with what was already on disk — so a `--domain`/`--segment`-scoped invocation sharing the same `--out-dir` as an earlier full run already destroys those other domains'/pairs' output rows. Carrying their old `comparison_registry.csv` stamp forward would falsely claim that data is still current when it no longer exists. `comparison_registry.csv` mirrors the same full-snapshot semantics: a scoped run correctly leaves every non-recomputed (pair, domain) with **no row at all**, so the next `--dry-run` reports it stale (never computed) rather than current — matching reality.

A (pair, domain) work item that produced no output at all this run — `run_pair()`/`_run_pair_domain()` returned `None` (e.g. a domain below `--min-patterns`, or a within-project pair with no eligible file pairs) and no governance-state rows were produced either — is also omitted; it must not get a "current" stamp for data that was never written.

A (pair, domain) is likewise omitted if either side's `run_registry.csv` `status` is not `"complete"`. `build_segment_manifest.py` updates a segment's `population_hash` to reflect its new file population immediately on manifest rebuild, resetting `status` to `"pending"` (and clearing `last_run_utc`) until the orchestrator actually re-runs that segment — but its output folder on disk still holds the *old* population's results until then. A compare run in that window reads the stale on-disk data, yet would otherwise get stamped with the segment's already-updated (new) `population_hash`; once the segment finally reaches `"complete"` with that same hash, a naive stamp would make a later `--dry-run` wrongly report the pair as already current.

...
|---|---|---|---|---|
| `project_union_jaccard_matrix.csv` | `union_jaccard` | `cross_segment_union_inventory.csv` | Jaccard between normalized project-level `join_hash` unions. It answers whether project/client scopes contain or use the same canonical patterns. | Do not read this as typical file-to-file similarity. It can differ from mean file-pair Jaccard. Missing union inventory blocks this output with an explicit status row. |

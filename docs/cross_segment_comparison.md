...
The explicit matrix outputs distinguish `missing`, `unavailable`, `not applicable`, and true zero-valued scores through `value_status` and blank values. Diagonal cells are synthetic self-comparisons (`self_comparison=true`) and are deterministic (`1.000000` for applicable similarity matrices).

...
Confirmed: there is **no naming-convention auto-discovery** for the real extraction pipeline (`runner/run_dynamo.py`). Domain wiring is fully explicit, in 4 places:
...
A new `domains/worksets.py` would need all 4 steps done by hand — nothing auto-discovers a new `domains/*.py` file.
...
Identical to Area 3 — no naming-convention auto-discovery for `run_dynamo.py`; a new `domains/browser_organization.py` needs the same 4 manual steps (import, `_enabled()`/`_domain_run()` block, `contracts/domain_identity_keys_v2.json` entry, join-key policy entry if needed). `probe_thin_runner.py`'s glob discovery already picks up `tools/probes/probe_browser_organization.py` for probe runs, same caveat as Area 3 that this has no bearing on real-pipeline wiring.
...
**Nullability caveat — the available probe data is limited to one project.** All 3 probe captures in `tools/probes/Exports/` come from the same document ("Kaiser Permanente San Rafael Replacement Hospital"), run at different times — not 3 different projects/firms. So while this confirms `Office` is shared-parameter-backed and therefore *would* be null on a non-Stantec-template project, the probe data cannot directly demonstrate cross-project nullability for the other fields, since every sample already has Stantec's parameter file loaded. This is an explicit open question a follow-up PR should flag rather than assume: for the true built-in fields (Client Name, Project Number, etc.), nullability across firms isn't at issue (they're native `ProjectInfo` parameters present in every Revit project by default) — but for any other shared-parameter-backed field found beyond `Office`, the same caveat applies and wasn't independently re-verified per-field here.
...
`CLAUDE.md`'s own Warnings section still says: *"The `phase_graphics` domain is intentionally disabled - do not attempt to enable without API justification."* Nothing found in this pass constitutes that justification — see below.
...
**`tools/probes/probe_phase_graphics.py` (full file read, 1039 lines) does NOT exercise the API surface the prompt describes.** Grepped the file for `GetCategoryOverrides`, `GetFilterOverrides`, `GetWorksetVisibility`, `GetCategoryHidden`, `GetLinePatternOverride`, `OverrideGraphicSettings` — **zero matches**. What the probe actually does:
...
None of this reads "V/G Overrides Model/Annotation/Filters/Import/RVT Links/Worksets" as override *state* — it reads ordinary parameters plus a filter/phase-filter membership crosswalk. The actual dedicated override-reading Revit APIs (`GetCategoryOverrides`, `GetCategoryHidden`, `GetFilterOverrides`, `GetWorksetVisibility`) are **not called anywhere in this probe file**.
...
So if the actual target for a "phase_graphics reimplementation" is per-view V/G overrides for Model/Annotation/Filters/Worksets, **that data is already captured today by `view_category_overrides_model.py`, `view_category_overrides_annotation.py`, and `view_templates.py`** — none of it is missing or gapped in the way the prompt implies. The only pieces of the named list with no current coverage anywhere in the repo are **Import categories** and **RVT Link** overrides (no domain or probe reads `GetCategoryOverrides`/equivalent scoped to imported-CAD or linked-model categories).

**Conclusion — this needs a scoping decision before any design write-up is actionable:** "phase_graphics" as named in `DECISIONS.md`/`CLAUDE.md`/the current domain file means the **global per-phase-status override table**, which D-013 found has no API access at all (not "not yet implemented" — API-inaccessible). The prompt's area 6 describes investigating **per-view** V/G overrides conditioned on Phase/Phase Filter, which is a different Revit concept, already mostly covered by other domains, and not what the current `phase_graphics.py`/its probe are built around. A "reimplementation" can't proceed on design questions (per-view vs. aggregated record shape) until it's decided which of these two things is actually being reimplemented — and if it's the first (the global table), someone needs new evidence overturning D-013's finding, since nothing produced in this pass provides that.
...
- **Option B — per-view record**, `record_id` keyed by view `UniqueId` (the existing convention for element-backed, identity-persistent entities per `CLAUDE.md`'s UniqueId Usage rule) or by view Name-based composite if templates are the intended scope (matching `view_category_overrides_model.py`/`view_templates.py`'s existing per-template record pattern). Only sensible if the target is per-view V/G overrides — and even then, largely redundant with `view_category_overrides_model.py`/`_annotation.py`/`view_templates.py` for everything except Import/RVT Link categories (per 6.1).
- **Option C — narrow the scope to just the genuinely-uncovered gap**: Import-category and RVT-Link overrides only, per view or per template, using the same `GetCategoryOverrides`/`GetCategoryHidden` pattern already proven in `view_category_overrides_model.py`. This is the option best supported by what this pass actually found (a real, narrow gap) rather than a broad reimplementation of already-covered ground — but is still a genuinely new scope decision, not something already decided by precedent.
...
### 7.0 Premise mismatch (same as elsewhere in this document)

`tools/probes/PROBE_EXPORTER_DELTA.md` does not exist — re-confirmed independently (`find`/`ls` against `tools/probes/`, zero hits). The only field-inventory sources that actually exist are `tools/probes/probe_dimension_types.py` and `tools/probes/Exports/PROBE_INVENTORY.csv`/`.md` (aggregated across 3 runs), plus the raw per-run JSON in `tools/probes/Exports/probes_2025_*.json` (which carries `observed_on_families`, not present in the CSV/MD rollups). All findings below are derived from these instead of the named-but-absent file.
...
Because every existing record is keyed to a spec label, adding these 3 doc-level fields requires **a new synthetic doc-level record** (e.g. `record_id = "units:_doc"`) — none of the current 38 per-spec records is a natural home for doc-scoped settings.
...
- `parent_name` → **already computed in-loop**, line 278: `parent_name = canon_str(getattr(parent if is_subcategory else cat, "Name", None))` — used today to build `row_key` (296) and the record label, just not currently emitted as its own identity item. It's derived from the tuple's `parent` value tracked during `_collect_categories` iteration, not from `cat.Parent` — mirrors how the probe's `c.parent_name` (`probe_object_styles.py:284`) also uses the tracked `parent` variable rather than `Category.Parent` (the probe's own reflection sweep shows `refl.Category.Parent` returns `q=missing` for top-level categories, so deriving from the tracked-parent variable is the only reliable path)
...
So: `can_add_subcategory`/`has_material_quantities`/`is_cuttable` are additional `getattr(cat_obj, "...", None)` reads inserted into the existing loop body (near the other `cat_obj`-derived items, lines 300-386); `parent_name` is already computed in-loop and just needs promotion to an identity item; `tab` needs no new Category read at all. **No new `FilteredElementCollector` pass and no separate category-lookup pass required for any of the 5.**
...
- **No code, test, or config file was created or modified** other than this document.

## Cross-area pattern worth calling out once, not per-area

Three separate areas (2, 6, 10) and one within-area finding (7's `leader_arrowhead_sig_hash`) turned out to already be implemented, reserved-but-unimplemented, or resting on a probe that doesn't test what the prompt assumed it tests. Before starting any of the 12 downstream PRs, re-run a targeted grep for the specific field/BuiltInParameter name across `domains/`, `contracts/domain_identity_keys_v2.json`, and `policies/*.json` — this document did that check everywhere it could, but a PR author should re-verify against the repo state at the time they actually start, since this document is a snapshot as of 2026-08-07.

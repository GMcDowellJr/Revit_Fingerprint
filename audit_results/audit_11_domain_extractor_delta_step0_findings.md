...
**Revised bottom line**: content decoding for Model/Annotation/Filters/Worksets already exists elsewhere in the repo (`view_category_overrides_model.py`/`_annotation.py`/`view_templates.py`, per the citations above) using the real dedicated APIs — the presence-only signal this probe found adds nothing beyond confirming those parameters are non-null on ~91% of the sampled views (136/150), which was never in doubt. The genuinely uncovered gap is still **Import-category and RVT-Link overrides** — no domain or probe anywhere in the repo calls `GetCategoryOverrides`/equivalent scoped to those two, and the presence-only `p.V/G Overrides Import`/`p.V/G Overrides RVT Links` rows don't change that; they only confirm the parameter exists, not what it contains.

**Conclusion — this still needs a scoping decision before any design write-up is actionable:** "phase_graphics" as named in `DECISIONS.md`/`CLAUDE.md`/the current domain file means the **global per-phase-status override table**, which D-013 found has no API access at all (not "not yet implemented" — API-inaccessible), and nothing in this pass or in the newly-reviewed delta document changes that. The prompt's area 6 describes investigating **per-view** V/G overrides conditioned on Phase/Phase Filter, which is a different Revit concept, already mostly covered by other domains for Model/Annotation/Filters/Worksets, and only genuinely gapped for Import/RVT Links. A "reimplementation" can't proceed on design questions (per-view vs. aggregated record shape) until it's decided which of these two things is actually being reimplemented — and if it's the first (the global table), someone needs new evidence overturning D-013's finding, since nothing produced in this pass (including the newly-added delta document) provides that.
